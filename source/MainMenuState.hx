...
class MainMenuState extends MusicBeatState
{
	...
	public static var ksliceVersion:String = '0.1.0';
	...
	override function create()
	{
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 64, 0, "K-Slice Engine v" + ksliceVersion, 12);
		...
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
		var versionShit:FlxText = new FlxText(12, FlxG.height - 24, 0, "VS Kyanite V1");
		versionShit.scrollFactor.set();
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
	...
	override function update(elapsed:Float)
	{
		if (FlxG.sound.music.volume < 0.8)
		{
			...
			if(freeplay.FreeplayState.vocals != null) freeplay.FreeplayState.vocals.volume += 0.5 * elapsed;
		}

		var lerpVal:Float = CoolUtil.boundTo(elapsed * 7.5, 0, 1);
		// camFollowPos.setPosition(FlxMath.lerp(camFollowPos.x, camFollow.x, lerpVal), FlxMath.lerp(camFollowPos.y, camFollow.y, lerpVal));

		if (!selectedSomethin && !controls.isInSubstate)
		{
			if (controls.UI_UP_P)
			{
