...
class FreeplayState extends MusicBeatSubstate
{
	...
	private var grpSongs:Array<String> = [];
	...
	public function new(?params:FreeplayStateParams, ?stickers:StickerSubState, ?filterony:Bool = false)
	{
	...
	override function create()
	{
		...
		disc = new FlxSprite().loadGraphic(Paths.image('freeplay/disc'));
		disc.scale.set(0.4, 0.4);
		disc.updateHitbox();
		disc.antialiasing = ClientPrefs.globalAntialiasing;
		disc.screenCenter(Y);
		disc.x = FlxG.width / 2 - (320 * 2);

		discBG = new FlxSprite(disc.x / 2 + 90, FlxG.height / 2 - 80).makeGraphic(Std.int(disc.width / 2), 100, 0xFF001928);
		add(discBG);

		add(disc);

		songText = new SongText(0, disc.height / 2 - 80, "Test", 40);
		songText.x = (FlxG.width / 2 + 10) - (disc.width / 2);
		add(songText);
		...
		#if PRELOAD_ALL
		var leText:String = LanguageFile.getPhrase("freeplay.tip");
		var size:Int = 16;
		#else
	...
	override function update(elapsed:Float)
	{
		...
		scoreText.text = '${LanguageFile.getPhrase("freeplay.personal_best")}: ' + lerpScore + ' (' + ratingSplit.join('.') + '%)';
		positionHighscore();
